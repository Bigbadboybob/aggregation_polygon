...
const factoryAddress = coinDATA.JETSWAP_FACTORY_ADDRESS
const factoryABI = coinDATA.JETSWAP_FACTORY_ABI
const quoteABI = coinDATA.JETSWAP_ABI
const quoteAddress = coinDATA.JETSWAP_ADDRESS

const pairABI = coinDATA.JETSWAP_PAIR_ABI
...
function getSwapAddr() {
    return coinDATA.JETSWAP_ADDRESS
}

function jetData(takerToken, makerToken, takerAmount) {
    return quoteContract.methods.getAmountsOut(takerAmount.toString(), [takerToken, makerToken]).call()
}
jetData(WMATIC, DAI, DN(10).pow(18)).then(res => console.log(res))
...
async function getRates(takerDollars) {
    ...
    for (let t = 0; t < coins.length; t++) {
        ...
        for (let m = 0; m < coins.length; m++) {
            ...
            let prom = jetData(allCoins[t], allCoins[m], takerAmount).then( res => {
