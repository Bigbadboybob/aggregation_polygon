...
const {quote, quickTrnscData} = require("../src/quickswap")
...
contract("MyV2FlashLoan", (accounts) => {
  ...
  it('flash loan', async () => {
    ...
    const callData = quickTrnscData(TOKEN, TOTOKEN, data)
    ...
    await token.approve(QUICKSWAP_ADDRESS, takerAmount, {from: account})

    const tx = await web3.eth.sendTransaction(
      {
        from: account, to: QUICKSWAP_ADDRESS,
