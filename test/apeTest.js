...
const {quote, apeTrnscData} = require("../src/apeswap")
...
contract("MyV2FlashLoan", (accounts) => {
  ...
  it('flash loan', async () => {
    ...
    const callData = apeTrnscData(TOKEN, TOTOKEN, data)
    ...
    await token.approve(APESWAP_ADDRESS, takerAmount, {from: account})

    const tx = await web3.eth.sendTransaction(
      {
        from: account, to: APESWAP_ADDRESS,
